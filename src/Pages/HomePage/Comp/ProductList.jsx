...
export default function ProductList() {
  ...
  const [products, setProducts] = useState(
    initialProducts.map((product) => ({
      ...product,
      imgLoading: true, // Set initial image loading state
    }))
  );
  ...
  return (
    <div>
      <h1 className="container mx-auto px-4 lg:px-24 md:text-2xl lg:text-3xl my-3 mt-8 font-semibold">
          ...
          <div key={product.id}>
            <div className="h-48 md:h-72 overflow-hidden relative">
              ...
              <div className="absolute top-3 right-3 bg-white w-6 h-6 md:w-9 md:h-9 rounded-full flex items-center justify-center">
                <i className="ri-heart-3-fill text-rose-700/20"></i>
              </div>
            </div>
            <h1 className="mt-2 text-sm md:text-lg line-clamp-1">
              {product.name}
            </h1>
